...
☝️ Mel이란? 사람의 달팽이관을 모티브로 따온 값
   - 달팽이관을 길게 펴서 보면 각 부분이 각기 다른 진동수를 감지함 -> 이 진동수를 기반으로 사람은 소리를 인식함
   - 낮은 주파수의 변화는 잘 감지하는데 높은 대역의 주파수 변화는 잘 감지하지 못함

☝️ Mel Scale이란? 사람 귀가 느끼는 방식에 맞게 주파수를 재배치한 스케일
   - 선형 주파수를 비선형 Mel 단위로 변환
   - 저주파는 촘촘하게, 고주파는 널널하게 배치됨

☝️ melwargs란? 멜 스펙트로그램을 만들 때 쓰는 설정들
    멜 스펙트로그램 생성 과정: [프레임 분할, FFT 수행, 멜 필터 적용, log 적용] -> 이후 DCT 수행을 통해 MFCC가 됨
    -> n_mels: FFT로 얻은 파워 스펙트럼을 멜 스케일로 압축할 때 쓰는 멜 필터 개수
               n_mel=40이면 한 프레임이 40차원의 log-mel 벡터가 됨 

☝️ 멜 스펙트럼이란?
    Ex. 0Hz, 1Hz, 2Hz, 3Hz, ... 8000Hz 원래 FFT 스펙트럼이 이런식으로 촘촘하게 생겼다면,
        멜 필터는 저음은 좁게 [0~100Hz], 고음은 넓게 [7000~8000Hz] 이런식으로 mel 축에서 등간격이 되도록 삼각형 필터를 배치함
        -> 각 필터에 대해 ∑(스펙트럼 값 x 멜 필터 가중치)를 계산하면 하나의 멜 에너지가 됨
           최종 [ 5.1, 3.2, 0.8, 6.7, ... ]   ← 40개
        -> 각 멜 필터는 보통 삼각형 모양
...
☝️ DCT란?
log-mel 벡터를 코사인 기저로 변환하는 과정 = Mel 에너지들을 진동 성분으로 다시 분해하는 것

-> 각 진동 성분의 크기를 나타내는 값이 계수
   Mel 벡터와 특정 코사인 패턴을 곱해서 모두 더한 값이 하나의 계수
⎾ 계수 0번: 전체 평균 에너지
   계수 1번: 전체적인 스펙트럼 모양
   계수 2번: 급격한 세부 변화
⎿ ... 
...
#### 🔍 개념 정리
- WaveNet은 1D dilated convolution을 쌓은 구조 -> 시계열/음성 생성용으로 특화된 CNN
...
- WaveNet의 게이팅 구조 = tanh ⊙ sigmoid = candidate x gate
  - tanh는 출력 범위가 -1~1로 제한되어 있어 신호의 중심이 0이 되어 안정적인 feature 표현을 만듦
  - sigmoid는 0~1 범위의 값을 가져서 정보의 통과량을 조절하는 게이트 역할을 함 <br>
   ⎿ 0에 가까우면 거의 차단, 0.3이면 30% 통과.. 이런 방식
  - CNN의 활성화 함수인 ReLU 구조를 쓰지 않는 이유: 정보를 선택적으로 차단하는 기능이 없음
  - 일반 CNN은 정보 흐름을 조절하는 게 중요하지 않음 but WaveNet은 과거 정보 중 중요한 것만 선택함
    - 게이트가 정보 흐름 제어, 장기 정보 유지에 유리함 
<br>
...
#### 🔍 개념 정리
- 음성 인식 파이프라인
  - 특성 추출: 오디오 신호에서 MFCC, 필터 뱅크 등의 특성을 추출함
  - 음향 모델링: 추출된 특성을 음소 또는 문자 단위의 확률로 변환함
  - 언어 모델링: 인식된 단어들의 시퀀스에 언어적 제약을 적용함
  - 디코딩: 가장 확률이높은 텍스트 시퀀스를 결정함
- CTC: 입력과 출력의 정렬이 명확하지 않은 시퀀스-투-시퀀스 학습 문제를 해결하기 위한 손실 함수
  - 손실 함수를 사용할 때 주의점: [시간, 배치, 클래스 수] 형태의 입력 형식 필요
- 평가 성능
  1. 워드 에러율(WER): 단어 단위로 얼마나 틀렸는지 보는지표
     - 정답: I want to sleep. 에측: I have to sleep. 이면 단어 기준 25% 틀림
  2. 문자 에러율(CER): 문자 단위로 얼마나 틀렸는지 보는 지표
     - 정답: I want to sleep. 예측: I wanr to sleep. 이면 총 문자 수 15개(공백 포함) 중 1개 틀림 = 6.7%
