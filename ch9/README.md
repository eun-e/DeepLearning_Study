...
- 텍스트 전처리 기법
    ...
    5. 워드 임베딩: 단어를 실수 벡터로 변환하여 단어 간 의미적 유사성을 반영하는 기법
       ...
       - 문맥 기반 임베딩: 같은 단어라도 문맥에 따라 서로 다른 벡터 얻음
- 모델링 방법
...
#### ❓추가 학습 내용
1. BERT: Transformer의 인코더만 쌓아 올려 만든 양방향 언어 모델
   - 무언가를 새로 생성하는 것보다 문맥을 완벽하게 이해하는 것이 목적
   - 문장에서 일부 단어를 mask해서 가려진 토큰을 맞히는 학습을 함
3. GPT: Transformer의 디코더만 이용한 단방향 언어 모델
   - 주어진 앞 문맥으로부터 다음 단어를 생성하도록 사전 훈련되며, 주로 텍스트 생성에서 강점을 보임
   - 학습과 생성 모두 다름 토큰을 예측하는 방
